...
const _fetchTimeoutLongPool = 30000
...
export class DB {
  ...
  try { 
      ...
      if (this.live) emitEvent(EVENT_NEW_DOC, { table: this.tableName, id, _id: record._id, data: data, new: record._rev == "",preview: this._preview(data), source: 'local', synced: false }); } catch(e) {}
      return id;
    } catch (err) {
  ...
  async listenChanges() {
    const poll = async () => {
      try {
        ...
        let url = `${this.couchUrl}/${this.tableName}/_changes?feed=longpoll&since=now`
        console.log (url)
        const res = await this._fetchWithTimeout(url,{timeout: _fetchTimeoutLongPool});
        console.log ("entra")
        const text = await res.text();
        console.log (`respuesta res en ${this.tableName}` )
        console.log (text)
        if (text.startsWith("{")) {
          ...
          for (const c of changes.results) {
            ...
            if (this.live) emitEvent(EVENT_NEW_DOC, { table: this.tableName, data: doc.data, id: doc._id, doc, source: 'remote', synced: true });
          }
        } else {
