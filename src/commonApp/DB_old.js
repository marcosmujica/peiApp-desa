...
export class LocalDB {
      ...
      if (this.dbType === 'idb') {
        ...
        try {
          ...
          try {
            ...
            const tables = await this.db.execAsync(
              "SELECT name FROM sqlite_master WHERE type='table' AND (name='docs' OR name='meta')"
            );
            
            // expo-sqlite devuelve un array con los resultados
            if (!tables || !tables[0] || !tables[0].values || tables[0].values.length < 2) {
              console.error('Resultado de verificación de tablas:', JSON.stringify(tables));
              throw new Error('Error en la verificación de tablas SQLite');
            }

            // Verificar que podemos hacer operaciones básicas
            try {
              // Es mejor usar COUNT(*) que puede devolver 0 sin generar error
              const docsResult = await this.db.execAsync('SELECT COUNT(*) as count FROM docs');
              const metaResult = await this.db.execAsync('SELECT COUNT(*) as count FROM meta');
              
              if (!docsResult || !metaResult) {
                throw new Error('Error al verificar las tablas');
              }
              
              console.log("Base de datos SQLite verificada y funcional");
              console.log("Documentos en docs:", docsResult[0]?.values?.[0]?.[0] || 0);
              console.log("Registros en meta:", metaResult[0]?.values?.[0]?.[0] || 0);
            } catch (e) {
              // Solo deberíamos llegar aquí si hay un error real en la base de datos
              if (e.message.includes('no such table')) {
                throw new Error('Las tablas no se crearon correctamente: ' + e.message);
              }
              console.error("Error al verificar las tablas:", e);
              throw new Error('Error al verificar el acceso a las tablas: ' + e.message);
            }
          } catch (error) {
  ...
  async getWithHeader(_id) { 
    try{
      ...
      { rows = await this.db.getFirstAsync('SELECT * FROM docs WHERE deleted=0 and _id=?', [_id] )}
...
// Crear instancia
const db = new DB('mydb', true, 'http://localhost:5984');

// Agregar documento (sincronización automática)
await db.add(null, { name: 'John' });

// Actualizar documento (sincronización automática)
await db.update({ _id: 'doc1', name: 'Jane' });
